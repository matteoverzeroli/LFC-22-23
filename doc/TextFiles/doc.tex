...
\begin{itemize}
	...
	\item \texttt{SYMBOLTYPE\_TO\_IMPLEMENT}: il simbolo inserito è riconosciuto ma deve ancora essere implementato per la traduzione corretta. I simboli presenti nella lista \texttt{listSymbolToImplement} sono solo alcuni dei simboli riconosciuti da LTSpice che nella versione attuale della applicazioni non vengono ancora tradotti;
	...
	\item \texttt{DESCRIPTION\_POLARIZED\_ERROR}: è un caso particolare dell'errore precedente. Nel caso in cui il simbolo è \texttt{cap}, la descrizione è \texttt{Polarized Capacitor};\todo{scritto così sembra obbligatorio per tutti i cap}
	\item \texttt{DESCRIPTION\_POLARIZED\_NOCAPACITOR\_ERROR}: indica il caso in cui il simbolo è \texttt{cap} e ci sia solo la parola \texttt{Polarized}, mentre l'applicazione si aspetta \texttt{Polarized Capacitor};
	\item \texttt{TYPE\_ERROR}: i valori dell'attributo che segue \texttt{Type} dipende dal componente in questione. Nel caso in cui il componente è \texttt{shottky}, \texttt{zener}, \texttt{LED} allora bisogna indicare \texttt{diode}. Nel caso in cui, invece, è \texttt{polcap} il valore accettato è \texttt{cap};
	\item \texttt{SPICELINEVALUE\_ERROR}: un attributo che segue la parola \texttt{SpiceLine} non è corretto. Gli attributi riconosciuti sono \texttt{tol}, \texttt{pwr}, \texttt{Rser}, \texttt{Rpar}, \texttt{Cpar}, \texttt{Ipk}, \texttt{mfg}, \texttt{pn}, \texttt{V}, \texttt{Irms}, \texttt{Lser}, \texttt{type};
	...
	\item \texttt{MISS\_TYPEATTR\_ERROR}: indica la mancanza dell'attributo \texttt{Type}. Esso è necessario per i componenti \texttt{schottky}, \texttt{zener}, \texttt{LED}; \todo{significa questo vale?}
	\item \texttt{MISS\_DESCATTR\_ERROR}: indica la mancanza dell'attributo \texttt{Description}. Vale per il componente \texttt{varactor};\todo{cosa significa vale ? vedo che nel codice è commentato }
